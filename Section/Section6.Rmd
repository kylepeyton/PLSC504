...
#Y1_plot <- ggplot(gg_df, aes(x = Y1)) + 
#  stat_density(colour = "red", fill = "red", alpha = 0.4) + 
#  theme_bw() + ylab("")

#plot_grid(Y0_plot, Y1_plot)
```


\clearpage 
\section{Causal inference in finite population}
...
\clearpage 
\section{Causal inference in samples from a finite population}

Now we will introduce sampling uncertainty into the design. Suppose that we only have the budget to treat 200 students. The setup is otherwise the same. 
...
```{r}
# We only have a budget to treat 200 of students from the population. 
...
# Again, we do not get to observe this process, we only see Y and W.
#   What's random here? What's fixed?
Y <- Y1*S*W + Y0*S*(1-W)
...
```{r}
# This is not equal to 10. Is the estimator biased? 
nsims <- 100000
